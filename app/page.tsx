...
export default function Chat() {
  const [count, setCount] = useState([{ key: "Test", value: 0 }]);
  ...
  useEffect(() => {
    if (latestMessage != null) {
      ...
      messages[messages.length - 1].content =
        "<b>" + messages[messages.length - 1].content + "</b>";
      setMessages(newMessages);
    }
  }, [latestMessage]);

  const hashName = "counter";
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const dataArray = Object.keys(dataCount)
        .map((key) => {
          return { key: key, value: dataCount[key] };
        })
  ...
  return (
    <div className="h-full w-full overflow-hidden">
      <div className="text-9xl text-red-900">
        {count.map((item: any) => (
          <div onClick={() => increment([item.key])} key={item.key}>
            {item.key}: {item.value}
          </div>
        ))}
      </div>
